...
const EditProductForm = observer(() => {
    ...
    useEffect(() => {
        ...
        ApiService.getInfoAboutMyProduct(pathVariable.id)
        .then(resp => {
  ...
  <Header/>
      ...
      onSubmit={(values) => {
        ...
        ApiService.editProduct(pathVariable.id, name, description, category, city, price)
        .then(resp => {
        setEditProductId(resp.data.productId)
        })
...
{errors.city && <div style={{color:"red",marginTop:"-20px", fontSize:"12px", marginBottom: "5px"}}>{errors.city}</div> }
...

  {
    ...
    {
    successAlert.show
    ? <SuccessAlert style={{ position: "absolute", bottom: "2px"}}>Заказ обновлен</SuccessAlert>
    : ""
        }
